...
private List<Pedido> lista = new ArrayList<>();    public DAOPedido(){
    ...
    public List<Pedido> listById(int id) {
        return em.createQuery("SELECT e FROM Pedido + e WHERE e.dataCadastro= :id").setParameter("id", id).getResultList();
    }

    public List<Pedido> listInOrderNome() {
        return em.createQuery("SELECT e FROM Pedido e ORDER BY e.dataCadastro").getResultList();
    }
...
public static void main(String[] args) {
        ...
        for (Pedido produto : listaPedido) {
            System.out.println(produto.getIdPedido()+"-"+produto.getClienteCodigoCliente());
        }
    }}